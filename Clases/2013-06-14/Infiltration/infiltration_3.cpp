...
struct int128
{
	...
	inline void set (size_t i, bool v) {
		...
		v_aux = v;
		v_aux <<= i;
		fprintf(stderr, "\tn[%u]: %u (%llu)\t\t%llu", i, v, v_aux, n[in]);
		if (v)
			n[in] |= v_aux;
		else
			n[in] &= ~v_aux;
	...
	inline void operator |= (const int128 &m) {
		n[0] |= m.n[0];
		n[1] |= m.n[1];
	}
};
...
struct Control
{
	...
	inline void set (const size_t n, bool value) {c.set (n, value);}
...
struct Cell
{
	...
	inline int get_bit (size_t i) {
		bool b;
		int r;
		r = scanf ("%1u", &b);
		set (i, b);
		#ifdef DEBUG_GET_BIT
		fprintf(stderr, "%u%u ", b, control[i]);
		if (i == CELLS - 1)
			fprintf(stderr, "\n");
		#endif
		return r;
...
inline void sort_cells ()
{
	...
	for (i = 0; i < CELLS; ++i)
	{
		...
		for (j = i + 1; j < CELLS; ++j)
		{
			cell[i].set (j, cell_aux[cell[i].number][cell[j].number]);
			cell[j].set (i, 1 - cell[i][j]);
		}
	}
